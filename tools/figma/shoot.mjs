...
const run = ([name, j]) => new Promise((res) => {
  ...
  const c = spawn("node", [new URL("./cap.mjs", import.meta.url).pathname, j.route, "x", "x", name, j.w || 1512, j.h || 982], { env: { ...process.env, STEPS: j.steps || "", SEED: j.seed || "", SHOT: file, NOAUTH: j.noauth ? "1" : "" }, stdio: ["ignore", "pipe", "pipe"] });
