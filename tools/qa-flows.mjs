...
async function session(query, width = 1440, height = 900) {
  ...
  await p.addInitScript(() => {
    ...
    try { localStorage.setItem("ttt_demo_fresh", "0"); localStorage.setItem("ttt_demo_quicklogin", "0"); } catch {}
...
await flow("Web: a long turn is one run, and one block can be moved to another voice", "", async ({ p, nav }, expect) => {
  ...
  await p.click("[data-slot=popover-content] [data-add-speaker]"); await p.fill("input[aria-label='New speaker name']", "Kirill"); await p.click("[data-add-save]"); await p.waitForTimeout(400);
  await expect("the new voice joins the list, nothing assigned yet", async () => (await vis(p, "[data-slot=popover-content] [data-speaker-row]:has-text('Kirill')")) && (await vis(p, "[data-segment-id='4'] [data-speaker-trigger]:has-text('Alex Johnson')")));
  await p.click("[data-slot=popover-content] [data-speaker-row]:has-text('Kirill')"); await p.waitForTimeout(300);
...
await flow("Web: the header chip opens the speakers panel; rename, merge and add work from there", "", async ({ p, nav }, expect) => {
  ...
  await p.click("[data-add-speaker]"); await p.fill("input[aria-label='New speaker name']", "Priya Patel"); await p.click("[data-add-save]"); await p.waitForTimeout(400);
